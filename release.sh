...
echo "fomolol/web-scripts: Running semantic-release in --dry-run to see if we should trigger a lerna release."
yarn web-scripts release --dry-run | grep "${expected_release_message}"

if [ $? -eq 0 ]
then
  echo "fomolol/web-scripts: A release will be triggered."
  echo "fomolol/web-scripts: Configuring git for Github Actions Lerna publish..."
  git config --global user.email "no-reply@fomolol.com"
  git config --global user.name "GitHub Action"
  git remote set-url origin "https://${GH_USERNAME}:${GH_TOKEN}@github.com/fomolol/web-scripts.git"
  git checkout master
  echo "fomolol/web-scripts: Configuring npm for publishing..."
  echo "//registry.npmjs.org/:_authToken=$NPM_TOKEN" >> ~/.npmrc
  echo "fomolol/web-scripts: Attempting publish..."
  npx lerna publish --yes --force-publish --conventional-commits --create-release=github --registry=https://registry.npmjs.org
  exit $?
else
  echo "fomolol/web-scripts: No release will be triggered." >&2
  exit 0
fi